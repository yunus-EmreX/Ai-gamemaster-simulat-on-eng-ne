...
class GameState:
    ...
    def apply_turn(self, player_action: str, turn_data: Dict[str, Any]):
        ...
        if isinstance(departed, list):
            for dep in departed:
                ...
                if not clean_dep:
                    continue
                is_death = any(kw in clean_dep for kw in ["ölü", "öldü", "ceset", "katledildi", "vefat", "yok edildi"])
                for npc_name in list(self.npc_relationships.keys()):
    ...
    def clean_stale_npcs(self):
        """Removes dead NPCs and characters absent for 3 or more turns from active scene, but preserves alive characters in known_world_npcs."""
        dead_keywords = ["öldü", "ölü", "ceset", "katledildi", "hayatını kaybetti", "vefat", "yok edildi"]
        departed_keywords = ["ayrıldı", "gitti", "uzaklaştı", "kaçtı", "terk etti"]
        for npc_name, att_val in list(self.npc_relationships.items()):
            att_lower = str(att_val).lower()
            name_lower = npc_name.lower()
            # If name or attitude contains death keywords
            ...
            # If attitude contains departed keywords
            if any(kw in att_lower for kw in departed_keywords):
                self.remove_npc(npc_name)
                if npc_name in self.known_world_npcs:
