...
class TestRPGArchitecture(unittest.TestCase):
    ...
    def test_rule_enforcer_deterministic_clamping(self):
        """Validates that RuleEnforcer actually clamps LLM deltas deterministically."""
        from core.rule_enforcer import RuleEnforcer
        from core.simulation_engine import SimulationCore

        # Case 1: Bleeding in Deli Mode -> LLM returned +5 healing or 0, Python MUST clamp to <= -15
        gs = GameState()
        gs.status_effects = ["Kanama"]
        gs.mode_data = {"mode": "realistic", "strictness": "deli"}

        mock_llm_turn = {
            "story": "Yarandan kan sızıyor ama kendini iyi hissediyorsun.",
            "arbiter_verdict": "Hafif atlattın.",
            "state_updates": {
                "health_delta": 5,
                "mental_delta": 0
            }
        }
        enforced = RuleEnforcer.enforce_post_turn(gs, "İlerliyorum", mock_llm_turn)
        self.assertEqual(enforced["state_updates"]["health_delta"], -15)
        self.assertIn("Kanama Etkisi", enforced["rule_warnings"])

        # Case 2: Unearned Healing without medical item / action -> Reset to 0
        gs_healthy = GameState()
        gs_healthy.status_effects = ["Normal"]
        gs_healthy.inventory = ["Pusula", "Fener"]
        gs_healthy.mode_data = {"mode": "realistic", "strictness": "ironclad"}

        mock_unearned_turn = {
            "story": "Birden enerjin yerine geldi ve yaraların mucizevi şekilde kapandı.",
            "arbiter_verdict": "İyileştin.",
            "state_updates": {
                "health_delta": 25
            }
        }
        enforced_unearned = RuleEnforcer.enforce_post_turn(gs_healthy, "Koşmaya devam ediyorum", mock_unearned_turn)
        self.assertEqual(enforced_unearned["state_updates"]["health_delta"], 0)
        self.assertIn("Senaryo Zırhı Engellendi", enforced_unearned["rule_warnings"])

        # Case 3: Empty Weapon Shot -> Enforce dry-click fail
        gs_gun = GameState()
        gs_gun.inventory = ["Paslı Tabanca (0 mermi)"]
        sim_eval = SimulationCore.process_action("Düşmana ateş ediyorum", gs_gun.to_dict())

        mock_gun_turn = {
            "story": "Tetiği çektin.",
            "arbiter_verdict": "Vuruş başarılı.",
            "state_updates": {"health_delta": 0}
        }
        enforced_gun = RuleEnforcer.enforce_post_turn(gs_gun, "Düşmana ateş ediyorum", mock_gun_turn, sim_eval)
        self.assertIn("kuru bir tetik sesi", enforced_gun["arbiter_verdict"])
        self.assertIn("Boş silahla ateşleme engellendi", enforced_gun["rule_warnings"])

    def test_multi_session_manager_isolation(self):
        """Verifies that SessionManager isolates GameState and API keys between sessions."""
        import app as app_module
        sm = app_module.session_manager

        # Session User A
        sid_a, game_a = sm.get_or_create("user_alpha")
        sm.set_api_key("AIzaSy_KEY_A", sid_a)
        game_a.health = 45
        game_a.inventory = ["Kılıç", "Kalkan"]

        # Session User B
        sid_b, game_b = sm.get_or_create("user_beta")
        sm.set_api_key("AIzaSy_KEY_B", sid_b)
        game_b.health = 90
        game_b.inventory = ["Yay", "Ok (20 adet)"]

        # Assert full independence
        self.assertEqual(game_a.health, 45)
        self.assertEqual(game_b.health, 90)
        self.assertEqual(sm.get_api_key(sid_a), "AIzaSy_KEY_A")
        self.assertEqual(sm.get_api_key(sid_b), "AIzaSy_KEY_B")
        self.assertNotEqual(game_a.inventory, game_b.inventory)

        # API isolation check via HTTP header
        res_a = self.client.post("/api/gm/modify_stat", json={"health": 30}, headers={"X-Session-ID": "user_alpha"})
        data_a = json.loads(res_a.data.decode("utf-8"))
        self.assertEqual(data_a["state"]["health"], 30)

        res_b = self.client.get("/api/current_state", headers={"X-Session-ID": "user_beta"})
        data_b = json.loads(res_b.data.decode("utf-8"))
        self.assertEqual(data_b["state"]["health"], 90)

    def test_npc_boundary_matching_and_false_positives(self):
        """Ensures that regex word boundaries prevent false positive deaths and token collisions."""
        gs = GameState()

        # 1. 'Gönüllü müttefik' and 'Ölümüne sadık' should NOT falsely trigger death!
        gs.npc_relationships = {
            "Elif": "Gönüllü müttefik",
            "Boran": "Ölümüne sadık savaşçı",
            "Düşman Muhafız": "Öldü ve cesedi nehre düştü"
        }
        gs.known_world_npcs = {
            "Elif": {"status": "Aktif Sahnede"},
            "Boran": {"status": "Aktif Sahnede"},
            "Düşman Muhafız": {"status": "Aktif Sahnede"}
        }

        gs.clean_stale_npcs()

        # Elif and Boran MUST survive!
        self.assertIn("Elif", gs.npc_relationships)
        self.assertIn("Boran", gs.npc_relationships)
        self.assertNotEqual(gs.known_world_npcs["Elif"]["status"], "[Ölü - Hayatını Kaybetti]")
        self.assertNotEqual(gs.known_world_npcs["Boran"]["status"], "[Ölü - Hayatını Kaybetti]")

        # Düşman Muhafız MUST be removed as dead
        self.assertNotIn("Düşman Muhafız", gs.npc_relationships)
        self.assertEqual(gs.known_world_npcs["Düşman Muhafız"]["status"], "[Ölü - Hayatını Kaybetti]")

        # 2. Short name word boundary test: 'Can' must not trigger on 'canavar'
        self.assertFalse(GameState.is_npc_present_in_text("Can", "Karanlık ormanda devasa bir canavar belirdi."))
        self.assertTrue(GameState.is_npc_present_in_text("Can", "Can sessizce eğilip silahını kontrol etti."))
