...
def take_action():
    ...
    # Generate Saga Chronicle (chronological milestone memory of past scenes)
    ...
    # Token-optimized context window (last 10-12 messages / 5-6 turns):
    # Historical turns only need player action & story narrative, backed by saga chronicle
    ...
    # Deterministic Post-Turn Guarantee:
    # If the deterministic engine identified weapon ammo consumption, ensure inventory is properly updated
    inv_eval = sim_eval.get("inventory_eval", {})
    if inv_eval.get("has_inventory_interaction") and inv_eval.get("ammo_consumed", 0) > 0:
        ...
        if matched and updated:
            ...
            if matched in current_game.inventory and matched not in rem:
                rem.append(matched)
            if updated not in add and updated not in current_game.inventory:
                add.append(updated)

    # Apply turn updates to game state
    current_game.apply_turn(player_action, turn_json)

    # Auto-save mechanism every 2 turns (turn_count > 0 and turn_count % 2 == 0)
    autosaved_file = None
    ...
    # Update raw chat history with compact player action & story narrative (saves 85-95% tokens on subsequent turns)
...
def gm_remove_npc():
    ...
    return jsonify({"success": True, "state": current_game.to_dict(), "message": msg})
...
def god_prayer_action():
    ...
    decision = data.get("decision", "grant").strip().lower()  # grant, smite, ignore, twist
...
def trigger_auto_save():
    ...
    if force and (current_game.turn_count == 0 or current_game.turn_count % 2 != 0):
        # Force auto-save for testing or GM actions
...
def load_game():
    ...
    if data.get("api_key"):
        active_api_key = data["api_key"].strip()
