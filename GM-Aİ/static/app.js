...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function getSelectedModel() {
    if (modelSelect && modelSelect.value === "custom") {
      ...
      return customVal || "gemini-3.8-flash";
    }
    return modelSelect ? modelSelect.value : "gemini-3.8-flash";
  }
  ...
  function applyPresetById(presetId) {
    ...
    if (preset.mode_data) {
      ...
      if (preset.mode_data.model) {
        const hasOption = Array.from(modelSelect.options).some(o => o.value === preset.mode_data.model);
        if (hasOption) {
          ...
          if (customModelInput) {
            customModelInput.value = preset.mode_data.model;
            customModelInput.style.display = "block";
          }
        }
      }
